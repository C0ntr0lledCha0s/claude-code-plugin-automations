...
def validate_agent(file_path: str) -> tuple[bool, list[str]]:
    ...
    if 'tools' in frontmatter:
        ...
        if isinstance(tools, str):
            ...
            # BLOCK Task tool in agents (subagents can't spawn subagents - architectural constraint)
            if 'Task' in tool_list:
                errors.append(
                    "CRITICAL ERROR: Agents CANNOT use the 'Task' tool. This is an architectural constraint:\n\n"
                    "   Why? Agents run as subagents, and subagents cannot spawn other subagents.\n"
                    "   This prevents infinite recursion and ensures resource management.\n\n"
                    "   Fix: Remove 'Task' from the tools field.\n\n"
                    "   Alternative patterns:\n"
                    "   - Use a SKILL for orchestration (skills run in main thread, can use Task)\n"
                    "   - Use a COMMAND for multi-agent workflows (commands run in main thread)\n"
                    "   - Let the main thread coordinate agents directly\n\n"
                    "   See: claude-component-builder/skills/building-agents/SKILL.md for details"
                )
    ...
    # Valid named colors from Claude Code UI
    valid_colors = ['red', 'blue', 'green', 'yellow', 'purple', 'orange', 'pink', 'cyan']
    if 'color' in frontmatter:
        color = frontmatter['color']
        if not isinstance(color, str):
            errors.append(f"Invalid color type: must be a string (e.g., blue)")
        elif color.lower() not in valid_colors:
            errors.append(f"Invalid color '{color}': must be one of {', '.join(valid_colors)}")
    else:
        errors.append("Recommendation: Add 'color' field for visual identification in terminal (e.g., color: blue)")
